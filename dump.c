...
void handleDobby(char* input);

cJSON *TransitionToJSON(const Transition *t);
cJSON *EnemyToJSON(const Enemy *e);
cJSON *CardToJSON(const Card *card);

void SaveDeckToJSON(Card *deck, int deckSize, const char *filename);
...
int main()
{
    ...
    initDeck();
    SaveDeckToJSON(deck, TotalNumberOfCards, "deck.json");
    return 0;
    //deck
    //enter game loop
    while(gameLooper)
    {
        ...
        if (fgets(buffer, MAX_INPUT_UNIQUE, stdin) != NULL) {
            ...
            else if (strcmp(buffer, "dobby") == 0)
            {
                printf("Talk to Dobby: ");
                if (fgets(buffer, MAX_INPUT_UNIQUE, stdin) != NULL)
                {
                    formatInput(buffer);
                    handleDobby(buffer);
                ...
                {
                    printf("Error reading text for Dobby.\n"); 
                }
            }
...
void handleJovi(char* input) {
    ...
    cJSON_AddStringToObject(json_data, "model", "llama3"); // update model name per your setup, llama3, phi
...
void handleDobby(char* input) {
    ...
    char result[MAX_INPUT_UNIQUE_JOVI];  // Define a buffer to hold the fully qualified prompt
    // Safely format into the result buffer using snprintf
    snprintf(result, MAX_INPUT_UNIQUE_JOVI, "You are an elf that speaks in rhymes named Dobby, I say '%s', how do you respond?", input);
    //printf("result - %s\n", result); //test the wrapped message
    // Prepare JSON data using cJSON
    cJSON *json_data = cJSON_CreateObject();
    cJSON_AddStringToObject(json_data, "model", "llama3"); // update model name per your setup, llama3, phi
...
// Helper: Convert Transition to cJSON
cJSON *TransitionToJSON(const Transition *t) {
    cJSON *json = cJSON_CreateObject();
    cJSON_AddNumberToObject(json, "cardType", t->cardType);
    cJSON_AddStringToObject(json, "name", t->name);
    return json;
}

// Helper: Convert Enemy to cJSON
cJSON *EnemyToJSON(const Enemy *e) {
    cJSON *json = cJSON_CreateObject();
    cJSON_AddNumberToObject(json, "enemyType", e->enemyType);
    cJSON_AddNumberToObject(json, "health", e->health);
    cJSON_AddNumberToObject(json, "damage", e->damage);
    return json;
}

// Convert Card to cJSON
cJSON *CardToJSON(const Card *card) {
    cJSON *json = cJSON_CreateObject();
    cJSON_AddNumberToObject(json, "cardType", card->cardType);
    cJSON_AddBoolToObject(json, "isStore", card->isStore);
    cJSON_AddStringToObject(json, "name", card->name);
    cJSON_AddStringToObject(json, "description", card->description);

    // Linked Cards
    cJSON *linkedCards = cJSON_CreateArray();
    for (int i = 0; i < card->numLinkedCards; i++) {
        cJSON_AddItemToArray(linkedCards, TransitionToJSON(&card->linkedCards[i]));
    }
    cJSON_AddItemToObject(json, "linkedCards", linkedCards);

    // Linked Items
    cJSON *linkedItems = cJSON_CreateIntArray(card->linkedItems, card->numLinkedItems);
    cJSON_AddItemToObject(json, "linkedItems", linkedItems);

    // Linked Enemies
    cJSON *linkedEnemies = cJSON_CreateArray();
    for (int i = 0; i < card->numLinkedEnemies; i++) {
        cJSON_AddItemToArray(linkedEnemies, EnemyToJSON(&card->linkedEnemies[i]));
    }
    cJSON_AddItemToObject(json, "linkedEnemies", linkedEnemies);

    return json;
}

void SaveDeckToJSON(Card *deck, int deckSize, const char *filename) {
    cJSON *deckArray = cJSON_CreateArray();

    for (int i = 0; i < deckSize; i++) {
        cJSON_AddItemToArray(deckArray, CardToJSON(&deck[i]));
    }

    char *jsonStr = cJSON_Print(deckArray); // Pretty-printed JSON (or use cJSON_PrintUnformatted)
